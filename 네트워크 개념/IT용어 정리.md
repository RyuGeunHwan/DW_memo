...
```
1. 기존알고 있던 데이터베이스와 다른 점은 insert만 가능! delete,update가 없음.
2. 데이터 조작이 불가능하다.
3. 비트코인과 다른 개념!(비트코인을 블록체인으로 관리)
4. ex) A테이블에 정보를 올리면 B,C테이블과도 체인으로 묶여있기 때문에 B,C테이블에서도 A테이블에 올린 정보를 알 수 있다.
```
...
```
1. 데이터베이스의 데이터를 json으로 불러와 HTML에 그려주다. -> rendering
```
