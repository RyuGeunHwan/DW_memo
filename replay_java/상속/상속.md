...
자바 상속 한계
    1. 자바, 자바스크립트는 다중상속이 불가능(C++은 다중상속 허용)
        1-1) 다중상속이 불가능하면 어떤 일이 생기는지? (다이아몬드문제(Diamond of Death 현상)로 인하여 다중상속 불가능!)
            1-1-1) 같은기능(메소드)이 있다고 하여도 서브클래스에서 이미 다른 슈퍼클래스를 상속받는다고 하였을 경우
            또 다시 상속받을 수 없는 경우가 생긴다.
            (클래스는 비슷한 메소드끼리 묶어놓는 암묵적인 룰이 있다.)
            1-1-2) 한 서브클래스의 슈퍼클래스가 두개라면 메소드를 가져올 때 어느 슈퍼클래스의 메소드를 실행해야 될지 알 수가 없다.

***다중상속을 받는 법!
    1. 프로젝트가 커지면 커질수록 ***다중상속 많이 사용!
    2. 자바에서 interface를 제공해줌.(interface사용)
