...
-having 
-> 1.grouping한 애들에서의 필터링  
   2.group by 뒤에 나와야함  
-ex) 그룹핑 한 애들중에 (무엇을) 조회하시오.  
-select   
job,  
sum(sal) as '월급 총 합'  
-from emp  
...
>- grouping 문제  
--각 부서별(deptno) 그룹화 하여 최고로 급여가 높은 사람의 이름과 직업을 나타내시오.    
...
-group by   
deptno  
--각 부서별(deptno) 그룹화 하여 세일즈맨을 제외한  5000만원 이상인 사람의 이름과 직업을 나타내시오.  
-select   
max(sal),  
ename,  
job  
...
from emp  
where job = 'clerk';  
>- --comm이NULL이 아니고, 직업이 salesman 그리고 
